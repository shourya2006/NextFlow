    ...
    <div className="px-12 pb-12 w-full" onClick={() => setOpenDropdownId(null)}>
      <section className="pt-8">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 2xl:grid-cols-6 gap-x-6 gap-y-10 pb-5">
          {/* New Workflow Card */}
          <button className="group flex w-full flex-col items-start gap-3 text-left">
          ...
          {workflows.map((ele) => (
             <div key={ele.id} className="group flex w-full flex-col items-start gap-3 text-left cursor-pointer relative">
               <div className="relative border border-[#262626] bg-[#141414] flex aspect-[2/1.33] w-full items-center justify-center rounded-md transition-all duration-200 hover:border-[#404040]">
                 <button 
                 ...
                 {openDropdownId === ele.id && (
                   ...
                   >
                     <button className="flex items-center gap-3 px-3 py-2 text-[14px] text-zinc-200 hover:bg-[#1a1a1a] transition-colors text-left w-full">
